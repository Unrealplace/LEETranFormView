...
- (void)drawRect:(CGRect)rect {
    ...
    for (ACTranformPoint *rectPoint in self.allPointsArray) {
        if (rectPoint.pointId == 0) { // 找到第一个点开始画点
            ...
            ACTranformPoint * nextPoint = self.allPointsArray[1];
            [self.path2 moveToPoint:CGPointMake((firstPoint.x + nextPoint.point.x)/2.0, (firstPoint.y + nextPoint.point.y)/2.0)];
        } else {
            [self.path addLineToPoint:rectPoint.point];
        }
        
        if (index == 1) {
            ACTranformPoint * nextPoint = self.allPointsArray[index];
            ACTranformPoint * nextPoint2 = self.allPointsArray[2];
            [self.path3 moveToPoint:CGPointMake((nextPoint.point.x + nextPoint2.point.x)/2.0, (nextPoint.point.y + nextPoint2.point.y)/2.0)];
        }
        if (index == 3) {
            ACTranformPoint * nextPoint = self.allPointsArray[index];
            ACTranformPoint * nextPoint2 = self.allPointsArray[0];
            [self.path3 addLineToPoint:CGPointMake((nextPoint.point.x + nextPoint2.point.x)/2.0, (nextPoint.point.y + nextPoint2.point.y)/2.0)];
        }
        
        if (index == 2) {
            ACTranformPoint * nextPoint = self.allPointsArray[index];
            ACTranformPoint * nextPoint2 = self.allPointsArray[3];
            [self.path2 addLineToPoint:CGPointMake((nextPoint.point.x + nextPoint2.point.x)/2.0, (nextPoint.point.y + nextPoint2.point.y)/2.0)];
        }
        index++;
    }
