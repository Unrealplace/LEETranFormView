...
const static CGFloat ACRectBorderLength = 100;
const static CGFloat ACRectButtonSize   = 50;

@interface ACRectView () <ACRectPointButtonDelegate>

@property (nonatomic,strong)ACRectPoint    * currentPoint;
@property (nonatomic,strong)UIBezierPath   * path;
@property (nonatomic,assign)ACViewShape      viewShape;
...
{
    for (ACRectPointButton *pointButton  in self.allPointButtonArray) {
...
#pragma mark - init 这个是直接生成 对应的形状的初始化方法
...
#pragma mark - init 这个是用几个点 生成对应的形状的初始化方法
- (instancetype)initWithSuperViewBounds:(CGRect)superViewBounds PointArray:(NSArray <ACRectPoint *> *)points
{
    ...
    if (self) {
        self.backgroundColor = [UIColor clearColor];
        self.viewShape = ACViewShape_Rect;
        self.path      = [[UIBezierPath alloc]init];
        self.canSave   = YES;
...
- (void) createAllPointWithFrame:(CGRect)frame
{
    switch (self.viewShape) {
        case ACViewShape_triangle:
            [self createTriangleWithFrame:frame];
            break;
        case ACViewShape_Rect:
...
#pragma mark - drawRect 画图的相关操作方法
- (void)drawRect:(CGRect)rect
{
    ...
    for (ACRectPoint *rectPoint in self.allPointArray) {
...
#pragma mark - ACRectPointButtonDelegate 按钮移动的代理方法
- (void)touchMoveButton:(ACRectPointButton *)rectPointButton WithTag:(NSInteger)tag WithPoint:(CGPoint)point
{
    ...
    ACRectPoint *pointObj = self.allPointArray[tag];
...
- (void)createRectangleWithFrame:(CGRect)frame
{
    for (NSInteger index = 0; index < self.viewShape ; index ++) {
        ...
        ACRectPoint *currentpoint = [[ACRectPoint alloc] init];
        ACRectPointButton *PointButton = [[ACRectPointButton alloc]initWithFrame:CGRectMake(0, 0, ACRectButtonSize, ACRectButtonSize)];
        ...
        switch (index) {
            case 0:
            {
                center = CGPointMake( ACRectButtonSize / 2,  ACRectButtonSize / 2);
                point  = CGPointZero;
            ...
            case 1:
            {
                center = CGPointMake(ACRectBorderLength - ACRectButtonSize , ACRectButtonSize / 2);
                point  = CGPointMake(ACRectBorderLength, 0);
                currentpoint.point = point;
                PointButton.center = center;
            }
                break;
            case 2:
            {
                center = CGPointMake(ACRectBorderLength -  ACRectButtonSize , ACRectBorderLength - ACRectButtonSize);
                point  = CGPointMake(ACRectBorderLength, ACRectBorderLength);
                currentpoint.point = point;
                PointButton.center = center;
            }
                break;
            case 3:
            {
                center = CGPointMake(ACRectButtonSize / 2, ACRectBorderLength - ACRectButtonSize);
                point  = CGPointMake(0, ACRectBorderLength);
                currentpoint.point = point;
                PointButton.center = center;
            }
                break;
...
- (double)computePointAngelWithLastPoint:(ACRectPoint *)lastPoint currentPoint:(ACRectPoint *)currentPoint nextPoint:(ACRectPoint *) nextPoint
{
    //先根据坐标计算角所对应两条邻边的长度
    //用绝对值计算两边的长度
    ...
    //AC是 b AB是 c BD是 a
    double cosA =( AC*AC + AB*AB - BD*BD  ) / (2 * AC * AB);
...
- (BOOL)computeAllPointIsInOtherTriangle{
    ...
    for (int i = 0; i < self.allPointArray.count; i++) {
        ...
        ACRectPoint *point = allpoints[i];
        [allpoints removeObject:allpoints[i]];
        ACRectPoint *otherPoint1 = allpoints[0];
        ACRectPoint *otherPoint2 = allpoints[1];
        ACRectPoint *otherPoint3 = allpoints[2];
...
{
    BOOL isIntersect ;
    ACRectPoint *point0 = self.allPointArray[0];
    ACRectPoint *point1 = self.allPointArray[1];
    ACRectPoint *point2 = self.allPointArray[2];
    ACRectPoint *point3 = self.allPointArray[3];
...
- (void)panGestureRecognizerAction:(UIPanGestureRecognizer *)pan
{
    ...
    for (ACRectPoint *point in self.allPointArray) {
